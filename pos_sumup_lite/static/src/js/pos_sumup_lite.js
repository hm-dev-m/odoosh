...
var deviceIsIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
...
pos_model.load_fields("pos.payment.method", [
    ...
    'sumup_default_customer_for_impersonal_orders',
]);
...
PosComponent.prototype.showScreen = function(name, props) {
    ...
    if (props !== undefined && name === 'ProductScreen') {
        ...
        if (is_sumup_transaction) {
            ...
            // Remove impersonal Customer if set
            const customer = currentOrder.get_client();
            if (customer) {
                const payment_method = currentOrder.selected_paymentline.payment_method;
                const impersonal_client = payment_method.sumup_default_customer_for_impersonal_orders;
                if (impersonal_client && impersonal_client[0] === customer.id) {
...
const PaymentScreenSumup = (PaymentScreen) =>
    class extends PaymentScreen {
        ...
        button_sumup_start_onclick() {
            ...
            // Whether a customer is mandatory for payment method (new in v15)
            const payment_method = this.selectedPaymentLine.payment_method;
            if (payment_method.split_transactions && !this.currentOrder.get_client()) {
                const impersonal_client = this.env.pos.db.get_partner_by_id(payment_method.sumup_default_customer_for_impersonal_orders[0]);
                if (impersonal_client) {
                    this.currentOrder.set_client(impersonal_client);
                } else {
                    this.showPopup('ErrorPopup',{
                        'title': _t('Customer Required!'),
                        'body': _t("Set Customer in this PoS Order or fill a field 'Default customer for impersonal orders' in current payment method's form and restart PoS session."),
                    });
                    return;
                }
            }
