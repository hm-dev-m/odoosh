...
ACCOUNT_TYPES = [
    ...
    'account.data_account_type_depreciation', 'account.data_account_type_non_current_assets',
    'account.data_account_type_fixed_assets', 'account.data_account_type_current_assets',
        ...
        for dict in list:
            ...
            for k, v in dict.items():
                ...
                if v['type'].type == 'cost1' and v['import_value']:
                    if debit_move_line['account_id'].user_type_id.id in account_types:
                        debit_move_line['analytic_account_id'] = self.get_object(v['type'].object, v['type'].field, v['import_value'], v['padding']).id
                    if credit_move_line['account_id'].user_type_id.id in account_types:
            ...
            if tax_id:
                ...
                if not tax_direction:
                    if credit_move_line['account_id'].user_type_id.id in self.get_account_types():
                        tax_direction = 'credit_move_line'
                if debit_move_line['account_id'].user_type_id.id in self.get_account_types() and tax_direction == 'debit_move_line':
                ...
                if credit_move_line['account_id'].user_type_id.id in self.get_account_types() and tax_direction == 'credit_move_line':
